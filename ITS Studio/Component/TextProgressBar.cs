...
namespace ProgressBarSample
{
    ...
    public class TextProgressBar : ProgressBar
    {
        ...
        public Font TextFont = new Font(FontFamily.GenericSerif, 11, FontStyle.Bold | FontStyle.Italic);
        //public Font TextFont = new Font(FontFamily.GenericSerif, 11, FontStyle.Bold);
        ...
        private string _currProgressStr
        {
            get
            {
                ...
                return string.Format("{0:0%}", (double)Value / (double)Maximum);
                //return string.Format("{0:0%}", (double)Value / (double)Maximum));
            }
        }
        ...
        private string _percentageStr
        {
            get
            {
                ...
                //return string.Format("{0} %", (int)((float)Value - Minimum) / ((float)Maximum - Minimum) * 100);
                return string.Format("{0:0%}", (int)((float)Value - Minimum) / ((float)Maximum - Minimum));
            }
        }
            ...
            get
            {
                ...
                switch (VisualMode)
                {
                    ...
                    case (ProgressBarDisplayMode.TextAndCurrProgress):
                        ...
                        text = string.Format("{0} {1}", CustomText, _currProgressStr);
                        break;

                    case (ProgressBarDisplayMode.TextAndPercentage):
                        //text = $"{CustomText}: {_percentageStr}";
                        text = string.Format("{0} {1}", CustomText, _percentageStr);
                        break;
                }
